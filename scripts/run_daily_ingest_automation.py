...
Human-like India/remote fresher daily preset (niche-only, slow crawl, all day):
  python3 scripts/run_daily_ingest_automation.py --india-remote-fresher-day --disable-mongo-fallback
...
Full day + student report + sheet: use scripts/run_daily_final.py if present.
"""
...
PIPELINE = ROOT / "scripts/run_job_ingest_pipeline.py"
PILOT = ROOT / "scripts/pipeline/run_pilot_day_batches.py"


def main() -> int:
    if not PIPELINE.is_file():
        print(
            f"ERROR: Missing {PIPELINE}\n"
            "Restore the job-ingest pipeline from your feature branch, e.g.:\n"
            "  git stash\n"
            "  git checkout origin/tamanna/phase-1-features -- scripts/run_job_ingest_pipeline.py scripts/pipeline/run_pilot_day_batches.py scripts/crawl_jobs_from_sources.py\n"
            "  (and other ingest-related scripts as needed)",
            file=sys.stderr,
        )
        return 2
    ...
    parser.add_argument(
        "--no-append-sheet",
        action="store_true",
        help="Skip Google Sheets export at end of pipeline",
    )
    parser.add_argument(
        "--pause-low-sources",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Do not auto-switch to JSON-only flow when Mongo is down (fail if Mongo unavailable).",
    )
    parser.add_argument(
        "--sources-file",
        type=Path,
    ...
    parser.add_argument(
        "--reset-checkpoint",
        action="store_true",
        help="Restart crawl checkpoint from source_offset=0 for this run.",
    )
    parser.add_argument(
    ...
    if args.pause_low_sources:
        pause_script = ROOT / "scripts/pipeline/pause_sources_low_india_yield.py"
        if not pause_script.is_file():
            print(f"WARNING: {pause_script} not found; skipping pause-low-sources step.", file=sys.stderr)
        else:
            pause_cmd = [py, str(pause_script)]
    ...
    if int(args.spaced_batches) > 0:
        if not PILOT.is_file():
            print(
                f"ERROR: Spaced mode requires {PILOT}\n"
                "Restore from your feature branch or run a single batch without --spaced-batches / --all-day.",
                file=sys.stderr,
            )
            return 2
        pilot_cmd = [
    ...
    print(
        ">>> Step: run_job_ingest_pipeline (crawl → ingest → ML → Postgres → sheet, or JSON-fallback → sheet)"
    )
