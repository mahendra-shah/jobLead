...
from scripts.job_board_flow.merge_job_runs import _is_non_job_or_spam
...
def main() -> int:
    ...
    if args.reexport_verified_json:
        r = subprocess.run([py, str(ROOT / "scripts/job_board_flow/job_ingest/export_verified_jobs_json.py")], cwd=ROOT)
        if r.returncode != 0:
            return r.returncode
    ...
    if args.refresh_sheet:
        cmd = [
            py,
            str(ROOT / "scripts/job_board_flow/export_job_board_jobs_to_sheets.py"),
            "--jobs-json",
