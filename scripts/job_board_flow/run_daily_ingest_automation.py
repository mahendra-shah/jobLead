...
Recommended (workday, 10–15 sources per part, ~5–10 min between parts):
  python3 scripts/job_board_flow/run_daily_ingest_automation.py --all-day

Human-like India/remote fresher daily preset (niche-only, slow crawling, all day):
  python3 scripts/job_board_flow/run_daily_ingest_automation.py --india-remote-fresher-day

More control:
  python3 scripts/job_board_flow/run_daily_ingest_automation.py --all-day --spaced-batches 24 \\
    --batch-size 12 --sleep-min 300 --sleep-max 600 --max-wall-minutes 540

Examples:
  python3 scripts/job_board_flow/run_daily_ingest_automation.py
  python3 scripts/job_board_flow/run_daily_ingest_automation.py --pause-low-sources
  python3 scripts/job_board_flow/run_daily_ingest_automation.py --spaced-batches 8 --sleep-min 240 --sleep-max 420
...
def main() -> int:
    ...
    if args.pause_low_sources:
        pause_cmd = [py, str(ROOT / "scripts/job_board_flow/pipeline/pause_sources_low_india_yield.py")]
    ...
    if int(args.spaced_batches) > 0:
        pilot_cmd = [
            py,
            str(ROOT / "scripts/job_board_flow/pipeline/run_pilot_day_batches.py"),
            "--iterations",
    ...
    pipe_cmd = [
        py,
        str(ROOT / "scripts/job_board_flow/run_job_ingest_pipeline.py"),
        "--batch-size",
        str(args.batch_size),
        "--source-request-delay",
        str(args.source_request_delay),
