...
Run from project root:
  python scripts/job_board_flow/run_daily_job_board_pipeline.py
...
def main() -> int:
    ...
    if not args.skip_discovery:
        if not run(
            [py, "scripts/job_board_flow/discovery/pipeline_2_github.py", "--delay", "1.5"],
    ...
    if not run(
        [py, "scripts/job_board_flow/discovery/export_crawl_ready_sources.py"],
    ...
    if not run([py, "scripts/job_board_flow/merge_job_runs.py"], "Step 4: Merge job runs → jobs_master.json"):
        return 1

    if not run([py, "scripts/job_board_flow/export_job_board_jobs_to_sheets.py"], "Step 5: Export to Google Sheet"):
        return 1
