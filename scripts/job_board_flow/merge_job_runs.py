...
from scripts.job_board_flow.crawl_jobs_from_sources import _derive_segment_category, _is_digital_marketing_job
...
def _cap_per_source(jobs: list[dict], max_per: int) -> list[dict]:
    """Keep up to max_per highest-quality jobs per source_domain (sheet diversity)."""
    if max_per <= 0:
        return jobs
    buckets: dict[str, list[dict]] = {}
    for j in jobs:
        d = (j.get("source_domain") or "unknown").strip().lower() or "unknown"
        buckets.setdefault(d, []).append(j)
    out: list[dict] = []
    for _domain, lst in buckets.items():
        lst.sort(key=_quality_score, reverse=True)
        out.extend(lst[:max_per])
    out.sort(key=_quality_score, reverse=True)
    return out
...
def _quality_score(job: dict) -> float:
    ...
    if job.get("salary"):
        score += 2.0
    if _is_digital_marketing_job(job):
        score += 2.5
    return score
...
def _balance_ratio(
    ...
    *,
    ...
    dm_quota = max(0, int(min_digital_marketing))
    dm_sorted = sorted(
        [j for j in jobs if _is_digital_marketing_job(j)],
        key=_quality_score,
        reverse=True,
    )
    dm_chosen: list[dict] = dm_sorted[: min(dm_quota, len(dm_sorted), cap)]
    dm_keys = {id(j) for j in dm_chosen}
    pool = [j for j in jobs if id(j) not in dm_keys]
    cap_remain = cap - len(dm_chosen)
    ...
    target_nontech = int(round(cap_remain * 0.64))
    target_tech = cap_remain - target_nontech
    ...
    selected: list[dict] = list(dm_chosen) + selected_non + selected_tech
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Max jobs per source_domain after profile filter (0 = unlimited). Default 20 for sheet diversity.",
    )
    parser.add_argument(
        "--min-digital-marketing",
        type=int,
        default=5,
        help="Reserve up to N digital-marketing rows in the balanced export when available (default 5).",
    )
    ...
    # Re-derive segment/category using title + description (reduces 'Unknown' share).
    for job in all_jobs:
        seg, cat = _derive_segment_category(
            job.get("title") or "",
    ...
    before_cap = len(all_jobs)
    all_jobs = _cap_per_source(all_jobs, int(args.max_per_source))
    if int(args.max_per_source) > 0:
        print(f"Per-source cap ({args.max_per_source}): {len(all_jobs)}/{before_cap} jobs kept.")

    all_jobs = _balance_ratio(
        all_jobs,
        max_master_jobs=int(args.max_master_jobs),
        min_digital_marketing=int(args.min_digital_marketing),
    )
